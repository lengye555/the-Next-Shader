    ...
    vec3 RSM(vec3 worldPos_p, vec3 worldNormal_p) {
        int sampleCount = int(remapSaturate(length(worldPos_p), 0.0, shadowDistance, 12.0, 4.0));
        ...
        return rsm / f_sample * 240.0;
    }

    vec3 rsmBlur(sampler2D tex, vec2 uv, vec2 dir) {
        ...
        dx *= mix(1.0, 0.1, viewz / far);

        vec3 col = texture(tex, uv).rgb * weight[0];
        float c = weight[0];
        for(int i = 1; i <= 2; i++) {
            float fi = float(i);
            float w1 = weight[i];
